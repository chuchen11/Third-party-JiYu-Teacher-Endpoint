...
[![Release](https://img.shields.io/badge/release-2026.07.31_preview-c7ff5a)](#20260731-研究预览版)
...
这是当前协议研究成果的阶段性发布，重点是保留可复现的协议证据、可观察的运行日志和明确的兼容性边界。

| 状态 | 能力 |
|------|------|
| **已验证** | 教师发现与登录、保活、聊天、缩略图、黑屏与键鼠锁定、自动解锁、关机/重启、学生信息、进程/窗口管理、远程运行与打开网页 |
| **可用** | `4806/TCP` 连续屏幕观看、UMSP channel 10、H.264/JPEG 解封装、U/V 色度修正、`SPUC` 远端光标解析 |
| **部分验证** | `MCMD` 远程控制：鼠标移动和点击已验证；控制窗口会隐藏本地系统指针并用学生端光标记录校准位置 |
| **实验性** | 键盘采集与 `kind=16` UDP 包已验证发出，但学生端 `SendInput` 在不同系统/会话上的实际效果仍不稳定，不应视为已完成能力 |

> 连续观看和远程控制涉及官方学生端编码器、输入桌面与 Windows Hook。请先执行 `view_probe`，并只在隔离、已授权的测试环境中使用。
...
### 连续屏幕观看与远程控制
- **安全探测**：先用 `view_probe` 检查学生端 `4806/TCP` 是否监听；探测过程不发送 feature 8 或 SHCO。
- **连续观看**：发送 `MESS feature type=8` 后立即单次连接 `4806/TCP`，用 `SHCO` 选择 UMSP channel 10，避免错过 H.264 的 SPS/PPS/IDR 初始化数据。连接失败会立即停止并冷却 60 秒，不会循环重试。
- **视频解封装**：解析 `TKPC` 外层、12 字节桌面分片以及 `HHRF`（H.264）/`HJRF`（JPEG）记录，支持乱序与重复分片。
- **颜色还原**：极域 H.264 码流交换了 U/V 色度平面，显示时自动交换回来，原始码流保持不变。
- **实时显示**：H.264 从首帧开始固定使用 bundled FFmpeg + Pillow/Tkinter 内嵌窗口，避免从观看切换到控制时重启解码器并错过 SPS/PPS/IDR。最近 JPEG 和可选 H.264 裸流保存在桌面 `teacher_remote_view`。
- **远程鼠标**：通过 `COMD` 事务承载 `MCMD mode=2`，启动学生端模拟输入接收器并发送严格 28 字节 UDP 输入包；窗口按实际图像区域映射坐标，解析 `SPUC` 记录校准远端光标，并在投屏区域隐藏本地系统指针。
- **实验性键盘**：控制窗口通过 Tk 事件和 Windows 低级键盘 Hook 获取 VK、扫描码、扩展键与按下/释放状态，并构造 `kind=16` 包。教师端捕获与发包已经日志验证，但学生端执行效果尚未稳定验证；Alt 和 Windows 键仍会被拦截。
...
- Python 3.8+
- Pillow
- imageio-ffmpeg（连续 H.264 画面解码）
- OpenCV（JPEG/OpenCV 显示回退，可选）
...
连续观看的输出保存在桌面 `teacher_remote_view` 目录；设置
`TEACHER_VIEW_DUMP=1` 可同时保留完整 H.264 裸流；每次观看使用独立的时间戳文件，
不会与上一次的不完整码流混在一起。TCP 端口可用
`TEACHER_TCP_PORT`（默认 4806）覆盖。远控默认让学生端监听随机高位 UDP 端口；
如需固定端口可设置 `TEACHER_CONTROL_PORT`，设为 `0` 则保持自动选择。
`control` 会先等待屏幕 TCP 通道连接，再发送官方“Control Student”对应的
`MCMD mode=2`；连接失败时
不会发送远控包。紧急停用远控可设置 `TEACHER_ENABLE_REMOTE_CONTROL=0`。
H.264 观看从第一帧起就使用内嵌窗口；执行 `control` 时复用同一解码器，不会在码流
中途切换播放器。日志中的 `[RemoteControl] input #...` 只表示窗口捕获并发出了输入包，
不等同于学生端已成功执行，尤其是仍处于兼容性验证阶段的键盘输入。
feature 8 中的教师会话端点默认使用当前频道端口，可分别通过
`TEACHER_VIEW_LOCAL_PORT` 和 `TEACHER_VIEW_PEER_PORT` 覆盖；它们不是学生端的
`4806/TCP` 服务端口。若学生端已经闪退，应先重启并等待重新登录，然后先执行
`view_probe`；不要连续重复执行 `view`。

### 已知限制

- 连续观看依赖学生端编码器状态；连接失败后有 60 秒冷却，避免重复请求导致学生端异常。
- 远程鼠标已在当前测试环境验证，画面和 `SPUC` 光标反馈通常存在约 100 ms 的可见延迟。
- 键盘 `kind=16` 包的教师端捕获、字段和发送链路已经验证，但学生端 `SendInput` 效果仍受系统版本、输入桌面和 Hook 状态影响，目前标记为实验性。
- 控制模式默认使用官方 `MCMD mode=2`，会锁定学生端本地输入；可用 `TEACHER_CONTROL_MODE=0` 切换为 Share 模式，但兼容性行为可能不同。
...
| `view_probe <ip> [port]` | 仅探测学生端 TCP 观看端口，不发送观看请求 |
| `view <ip> [port]` | 连续观看学生屏幕（TCP/UMSP，默认端口 4806） |
| `view_stop <ip>` | 停止连续观看 |
| `control <ip> [port]` | 打开交互式远程控制窗口（默认随机高位 UDP 端口） |
| `control_stop <ip>` | 停止远程控制 |
| `mouse <ip> <动作> <x> <y> [data]` | 发送鼠标事件，坐标范围 0..65535 |
| `key <ip> <vk> [press\|down\|up] [scan] [extended]` | 发送 Windows 虚拟键事件 |
| `all` | 请求所有学生的屏幕预览 |
...
teacher> view_probe 192.168.2.139       # 先确认 4806/TCP 已监听
teacher> view 192.168.2.139             # 连续观看屏幕
teacher> control 192.168.2.139          # 开始远控
teacher> mouse 192.168.2.139 move 32768 32768  # 移动到画面中心
teacher> key 192.168.2.139 0x41 press   # 按一次 A 键
teacher> control_stop 192.168.2.139     # 停止远控
teacher> view_stop 192.168.2.139        # 停止连续观看
